...
import { mockAIservice, mockConteudoGeradoRepository, mockDisciplinaRepository, mockRagBnccService, mockUnidadeRepository, } from "../mocks/Mocks";
...
describe("Conteudo Use Cases", () => {
    let atualizarConteudoUseCase;
    let listarConteudosUseCase;
    let gerarUnidadeEConteudosUseCase;
    beforeEach(() => {
        jest.clearAllMocks();
        atualizarConteudoUseCase = new AtualizarConteudoUseCase(mockConteudoGeradoRepository);
        listarConteudosUseCase = new ListarConteudosUseCase(mockConteudoGeradoRepository);
        gerarUnidadeEConteudosUseCase = new GerarUnidadeEConteudosUseCase(mockAIservice, mockUnidadeRepository, mockDisciplinaRepository, mockRagBnccService, mockConteudoGeradoRepository);
    });
    describe("AtualizarConteudoUseCase", () => {
        it("Deve atualizar um conteúdo existente", async () => {
            ...
            const conteudoOriginal = new ConteudoGerado(conteudoId, unidadeId, "plano_de_aula", "Conteúdo original", new Date());
            ...
            const resultado = await atualizarConteudoUseCase.execute(conteudoId, novoTexto);
        ...
        it("Deve lançar erro ao tentar atualizar conteúdo inexistente", async () => {
            ...
            await expect(atualizarConteudoUseCase.execute(conteudoId, "Novo conteúdo")).rejects.toThrow("Conteúdo não encontrado.");
        });
        it("Deve lançar erro ao tentar atualizar sem ID ou conteúdo", async () => {
            await expect(atualizarConteudoUseCase.execute("", "Novo conteúdo")).rejects.toThrow("Dever ser informado o ID e o novo conteúdo para atualização.");
            await expect(atualizarConteudoUseCase.execute(randomUUID(), "")).rejects.toThrow("Dever ser informado o ID e o novo conteúdo para atualização.");
        });
        it("Deve atualizar conteúdo mantendo os outros campos", async () => {
            const conteudoId = randomUUID();
            const unidadeId = randomUUID();
            const dataOriginal = new Date();
            const conteudoOriginal = new ConteudoGerado(conteudoId, unidadeId, "atividade", "Conteúdo original da atividade", dataOriginal);
            const novoTexto = "Conteúdo da atividade totalmente revisado";
            const conteudoAtualizado = new ConteudoGerado(conteudoId, unidadeId, "atividade", novoTexto, dataOriginal);
            mockConteudoGeradoRepository.buscarPorID.mockResolvedValue(conteudoOriginal);
        ...
        it("Deve atualizar diferentes tipos de conteúdo", async () => {
            ...
            for (const tipo of tipos) {
                const conteudoId = randomUUID();
                const unidadeId = randomUUID();
                const conteudo = new ConteudoGerado(conteudoId, unidadeId, tipo, "Conteúdo original adequado para teste", new Date());
                const novoTexto = `Novo conteúdo para ${tipo}`;
                const conteudoAtualizado = new ConteudoGerado(conteudoId, unidadeId, tipo, novoTexto, new Date());
    ...
    describe("ListarConteudosUseCase", () => {
        it("Deve listar todos os conteúdos de uma unidade", async () => {
            const unidadeId = randomUUID();
            const conteudosMock = [
                new ConteudoGerado(randomUUID(), unidadeId, "plano_de_aula", "Este é um plano de aula completo", new Date()),
                new ConteudoGerado(randomUUID(), unidadeId, "atividade", "Esta é uma atividade completa", new Date()),
                new ConteudoGerado(randomUUID(), unidadeId, "slide", "Este é um conteúdo de slide", new Date()),
            ];
        ...
        it("Deve listar conteúdos com todas as propriedades corretas", async () => {
            ...
            const conteudosMock = [
                new ConteudoGerado(randomUUID(), unidadeId, "plano_de_aula", "Plano detalhado de matemática", dataEsperada),
            ];
            mockConteudoGeradoRepository.listarPorUnidade.mockResolvedValue(conteudosMock);
            const resultado = await listarConteudosUseCase.execute(unidadeId);
        ...
        it("Deve listar apenas conteúdos da unidade especificada", async () => {
            ...
            const conteudosMock = [
                new ConteudoGerado(randomUUID(), unidadeId1, "plano_de_aula", "Plano da unidade específica", new Date()),
                new ConteudoGerado(randomUUID(), unidadeId1, "atividade", "Atividade da unidade específica", new Date()),
            ];
            mockConteudoGeradoRepository.listarPorUnidade.mockResolvedValue(conteudosMock);
    ...
    describe("GerarConteudoEUnidadeUseCase", () => {
        it("Deve criar uma unidade com conteúdos gerados", async () => {
            ...
            const tipos = ["plano_de_aula", "atividade"];
            const disciplinaMock = new Disciplina(disciplinaId, "MA", "9_ANO");
            const unidadeCriadaMock = new Unidade(randomUUID(), disciplinaId, tema, new Date());
            ...
            mockConteudoGeradoRepository.criar.mockImplementation(async (conteudo) => {
                // Retorna o conteúdo com a data de criação definida
                return new ConteudoGerado(randomUUID(), conteudo.UnidadeID, conteudo.Tipo, conteudo.Conteudo, new Date());
            });
            const result = await gerarUnidadeEConteudosUseCase.execute(disciplinaId, tema, tipos);
            ...
            expect(result.conteudos.some((c) => c.tipo === "plano_de_aula")).toBe(true);
