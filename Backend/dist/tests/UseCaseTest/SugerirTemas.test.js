...
describe("SugerirTemasUseCase", () => {
    ...
    it("Deve sugerir temas para uma disciplina válida", async () => {
        ...
        expect(mockAIservice.sugerirTemasCulturaDigital).toHaveBeenCalledWith(expect.any(String), // nome da disciplina
    ...
    it("Deve lançar erro quando disciplina não existe", async () => {
        ...
        await expect(sugerirTemasUseCase.execute(disciplinaId)).rejects.toThrow("Disciplina não encontrada");
    });
    it("Deve sugerir temas para diferentes disciplinas", async () => {
        const disciplinas = [
            { id: randomUUID(), codigo: "LP", serie: "8_ANO", temas: ["Fake News", "Mídias Digitais"] },
            { id: randomUUID(), codigo: "CI", serie: "7_ANO", temas: ["Tecnologia Verde", "Robótica"] },
            { id: randomUUID(), codigo: "HI", serie: "6_ANO", temas: ["História Digital", "Museus Virtuais"] },
        ];
