...
import { mockAIservice, mockConteudoGeradoRepository, } from "../mocks/Mocks";
import { ConteudoGerado } from "../../02-domain/entities/ConteudoGerado";
import { randomUUID } from "node:crypto";
describe("RefinarConteudoPreviewUseCase", () => {
    let refinarConteudoPreviewUseCase;
    beforeEach(() => {
        jest.clearAllMocks();
        refinarConteudoPreviewUseCase = new RefinarConteudoPreviewUseCase(mockConteudoGeradoRepository, mockAIservice);
    });
    it("Deve gerar prévia de refinamento para plano de aula", async () => {
        const conteudoId = randomUUID();
        const unidadeId = randomUUID();
        const conteudoOriginal = new ConteudoGerado(conteudoId, unidadeId, "plano_de_aula", "Plano original", new Date());
    ...
    it("Deve chamar refinarAtividade para tipo atividade", async () => {
        const conteudoId = randomUUID();
        const unidadeId = randomUUID();
        const conteudoOriginal = new ConteudoGerado(conteudoId, unidadeId, "atividade", "Atividade original", new Date());
