...
export class OpenAIService {
     ...
     */
    async sugerirTemasCulturaDigital(disciplina, anoSerie) {
...
["Tema 1", "Tema 2", "Tema 3", "Tema 4", "Tema 5"]`;
     ...
     */
    async gerarPlanoDeAula(tema, contextoBNCC, disciplina, anoSerie) {
...
RETORNE APENAS O JSON, sem markdown wrappers.`;
        try {
            const response = await this.client.chat.completions.create({
                ...
                messages: [
                    {
                        role: "system",
                        content: "Você é um professor experiente especializado em BNCC e cultura digital. Retorne APENAS um objeto JSON válido com as propriedades 'planoDeAula' e 'conteudoSlides', ambas contendo texto em formato Markdown.",
                    },
     ...
     */
    async gerarAtividade(tema, contextoBNCC, disciplina, anoSerie) {
...
Seja CLARO e JUSTO.`;
        try {
            const response = await this.client.chat.completions.create({
                ...
                messages: [
                    {
                        role: "system",
                        content: "Você é um professor experiente especializado em avaliação e BNCC.",
                    },
                    { role: "user", content: prompt },
                ],
                temperature: 0.8,
    ...
    async refinarPlanoDeAula(planoDeAula, instrucoes) {
    ...
    **RETORNE UM OBJETO JSON** com a seguinte estrutura:
    ...
    async refinarAtividade(atividade, instrucoes) {
        if (!instrucoes || instrucoes.trim() == "") {
            return {
                conteudo: atividade,
    ...
    **RETORNE UM OBJETO JSON** com a seguinte estrutura:
    ...
    async refinarSlide(slideMarkdown, instrucoes) {
        if (!instrucoes || instrucoes.trim() == "") {
            return {
