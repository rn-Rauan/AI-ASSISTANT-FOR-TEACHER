...
export class GerarPPTXUseCase {
     ...
     */
    constructor(
        private conteudoRepository: IConteudoGeradoRepository,
     ...
     */
    async execute(conteudoID: string, tema?: string): Promise<Buffer> {
        ...
        const pptxBuffer = await this.slideService.gerarPPTX(
            conteudo.Conteudo,
            tema || "Apresentação"
        );
